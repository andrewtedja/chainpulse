...
export function SentimentHistory() {
  ...
  const chartData = (() => {
    ...
    return Object.entries(groupedByDate)
      .map(([dateKey, value]) => ({
        ...
        sentiment: value.scores.reduce((sum, score) => sum + score, 0) / value.scores.length,
        articleCount: value.scores.length,
        articles: value.articles,
        dateKey: dateKey, // Keep for sorting
      }))
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    ...
    return (
      <div className="bg-gray-900/95 backdrop-blur border border-white/20 rounded-lg p-3 shadow-xl min-w-[200px]">
        ...
        <p className="text-xs text-gray-500">
          {data.articleCount} article{data.articleCount !== 1 ? "s" : ""} analyzed
        </p>
      </div>
    );
  };
  ...
  return (
    <GlassCard>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <PeriodFilter activePeriod={period} onPeriodChange={setPeriod} periods={["24h", "7d", "all"]} />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="h-[300px] sm:h-[350px] flex items-center justify-center">
          <p className="text-gray-400 text-sm">No sentiment data available for the selected period</p>
        </div>
      ) : (
        <div className="h-[300px] sm:h-[350px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(222 20% 18%)" opacity={0.3} />
              <XAxis
                dataKey="time"
                axisLine={false}
                tickLine={false}
              ...
              <ReferenceLine y={0} stroke="hsl(222 10% 55%)" strokeDasharray="3 3" opacity={0.5} />
              <Area
                type="monotone"
                dataKey="sentiment"
